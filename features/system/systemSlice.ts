...
export const systemSlice = createSlice({
    ...
    reducers: {
        updateAffiliation: (
            state,
            action: PayloadAction<UpdateAffiliationPayload>
        ) => {
            ...
            if (p.regions && p.regions.length)
                state.meeter.region = p.regions[0];
            if (p.eventRegions && p.eventRegions.length)
                state.meeter.eventRegion = p.eventRegions[0];
            ...
            if (Object.keys(p).length)
                state.meeter.affiliate = { ...state.meeter.affiliate, ...p };
...
export const selectIsLoading = (state: RootState) =>
    state.system.isLoading || state.system.meeter.isLoading;
